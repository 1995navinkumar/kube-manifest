...
resource "kubernetes_manifest" "persistent-volume-3Gi_1" {
  manifest = yamldecode(file("${path.module}/manifests/persistent-volume-3Gi_1.yaml"))
}

resource "kubernetes_manifest" "persistent-volume-3Gi_2" {
  manifest = yamldecode(file("${path.module}/manifests/persistent-volume-3Gi_2.yaml"))
}

resource "kubernetes_manifest" "persistent-volume-3Gi_3" {
  manifest = yamldecode(file("${path.module}/manifests/persistent-volume-3Gi_3.yaml"))
}
